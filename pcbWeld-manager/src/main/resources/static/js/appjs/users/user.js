						...
						columns : [
								...
								*/
									...
									formatter : function(value, row, index) {
										...
										var ff = '<a class="btn btn-danger btn-sm" title="检查数据"  mce_href="#" onclick="acupuncture(\''
												+row.id
...
/*function acupuncture(id){
	var page = layer.open({
		...
		area : [ '450px', '520px' ],
		content : '/information/resultOptometry/'+id // iframe的url
	});
	layer.full(page)
}*/
function acupuncture(id){
	var page = layer.open({
		...
		shadeClose : false, // 点击遮罩关闭层
		area : [ '450px', '520px' ],
		content : '/information/checkData/'+id // iframe的url
	});
	layer.full(page)
}
function reLoad() {
